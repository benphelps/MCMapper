...
float cNoise::IntNoise2D( int a_X, int a_Y )
{
	int n = a_X + a_Y * 57;
    n = (n<<13) ^ n;
...
float cNoise::IntNoise3D( int a_X, int a_Y, int a_Z )
{
	int n = a_X + a_Y * 57 + a_Z * 57*57;
    n = (n<<13) ^ n;
    return ( 1.0f - ( (n * (n * n * 15731 + 789221) + 1376312589) & 0x7fffffff) / 1073741824.0f);  
}
 ...
 **/
float cNoise::LinearNoise1D( float a_X )
{
...
float cNoise::CosineNoise1D( float a_X )
{
	int BaseX = FAST_FLOOR( a_X );
	float FracX = (a_X) - BaseX;
...
float cNoise::CubicNoise1D( float a_X )
{
	int BaseX = FAST_FLOOR( a_X );
	float FracX = (a_X) - BaseX;
...
float cNoise::SmoothNoise1D( int a_X )
{
 ...
 **/
float cNoise::LinearNoise2D( float a_X, float a_Y )
{
...
float cNoise::CosineNoise2D( float a_X, float a_Y )
{
	const int	BaseX = FAST_FLOOR( a_X );
	const int	BaseY = FAST_FLOOR( a_Y );
...
float cNoise::CubicNoise2D( float a_X, float a_Y )
{
	const int	BaseX = FAST_FLOOR( a_X );
	const int	BaseY = FAST_FLOOR( a_Y );
 ...
 **/
float cNoise::CosineNoise3D( float a_X, float a_Y, float a_Z )
{
	const int	BaseX = FAST_FLOOR( a_X );
	const int	BaseY = FAST_FLOOR( a_Y );
...
float cNoise::CubicNoise3D( float a_X, float a_Y, float a_Z )
{
	const int	BaseX = FAST_FLOOR( a_X );
	const int	BaseY = FAST_FLOOR( a_Y );
	const int	BaseZ = FAST_FLOOR( a_Z );
